...
const string strMessageMagic = "EddieCoin Signed Message:\n";
...
if(nBestHeight < 10000) 
    ...
    {
           nSubsidy = nCoinAge * COIN_YEAR_REWARD12 * 33 / (365 * 33 + 8); // 12%
    }

else if(nHeight < 200000)
    {
           nSubsidy = nCoinAge * COIN_YEAR_REWARD7 * 33 / (365 * 33 + 8); // 7%
    }
else if(nHeight < 300000)
    {
           nSubsidy = nCoinAge * COIN_YEAR_REWARD5 * 33 / (365 * 33 + 8); // 5%
    }
    ...
    {
           nSubsidy = nCoinAge * COIN_YEAR_REWARD3 * 33 / (365 * 33 + 8); // 12%
    }
    ...
    {
           nSubsidy = nCoinAge * COIN_YEAR_REWARD3 * 33 / (365 * 33 + 8); // 10%
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EddieCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Eddie Coin V0.2 08-09-2017";
        CTransaction txNew;
        txNew.nTime = 1504808681;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1504808681;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 866087 : 866087;
        ...
        assert(block.hashMerkleRoot == uint256("0xc86dba7a740c94dc4f373abeffbb646f1bc4ec64c07408570d81a95907835610"));
